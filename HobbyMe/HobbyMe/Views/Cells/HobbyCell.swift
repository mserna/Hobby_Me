...
class HobbyCell: BaseCell {
    
    var hobby: Hobby? {
        didSet {
            titleLabel.text = hobby?.hobbyName
            
            setupThumbnailImage()
            
            //measure title text
            if let title = hobby?.hobbyName {
    ...
    func setupThumbnailImage() {
        if let thumbnailImageUrl = hobby?.image {
//            thumbnailImageView.loadImageUsingUrlString(urlString: thumbnailImageUrl)
    ...
    let thumbnailImageView: CustomImageView = {
        ...
        imageView.image = UIImage(named: "chess")
